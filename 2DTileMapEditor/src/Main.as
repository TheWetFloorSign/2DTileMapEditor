package  {
	...
	public class Main extends Sprite{
		...
		[Embed(source="../_raw/TileSheet.png")]
		private var TileSheet:Class;
		private var tileSheetBM:Bitmap = new TileSheet() as Bitmap;
		private var tileSheetData:BitmapData = tileSheetBM.bitmapData;
		...
		private function loadMap(map:Object = null):void
		{
			...
			sceneBMD = new BitmapData(mapSize.w * tileSize, mapSize.h * tileSize, false, 0xffffff);
		...
		public function createAssets():void{
			...
			curTileBMD = new BitmapData(tileSize,(tileSheetBM.width * tileSheetBM.height)/tileSize,false,0xffffff);
		...
		{
			var tempWidth:int = tileSheetBM.width / tileSize;
			var tempHeight:int = tileSheetBM.height / tileSize;
			var tempY:int = 0;
			var scale:int =2;
			for (var i:int = 0; i < tempHeight; i++)
			{
				for (var j:int = 0; j < tempWidth; j++)
				{
					...
					tileBMD = createBitmapData(tileSheetData, tileSize* j,tileSize* i,tileSize, tileSize, true);
		...
		public function popUsedTileSet():void{
			...
			for(var i:int = 0; i<jsonTileList.length;i++){
				...
				tileBMD.copyPixels(tileSheetData, new Rectangle(jsonTileList[i].frames[0].framePosition[0],jsonTileList[i].frames[0].framePosition[1],jsonTileList[i].frames[0].frameSize[0], jsonTileList[i].frames[0].frameSize[1]),new Point(0,0));
				tileDisplay = new Sprite();
				tileDisplay.name = String(tempY);
				var tileBM:Bitmap = new Bitmap(tileBMD);
				tileDisplay.addChild(tileBM);
		...
		public function updateCurTile():void{
			...
			curTileBMD.copyPixels(tileSheetData, new Rectangle(curTileData.frames[0].framePosition[0],curTileData.frames[0].framePosition[1],tileSize,tileSize),new Point(0,0));
		}
